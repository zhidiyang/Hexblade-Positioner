function [YorN] = BuckledElementsTimoshenko(Constraint,Twist)
%Function reads in a matrix (Constraint) and a set of twists on the nodes
%and returns an answer as to whether the structure has buckled.
...
for i = 1:wire
    ...
    Answer = Buckled(Constraint(i,:),transpose(TotalForces));   %%%%%%%%%%%%Function
    if (Answer==1)
        count = 1;
        break;
    end
